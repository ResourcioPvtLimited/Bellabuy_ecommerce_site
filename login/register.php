...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (empty($name)) {
    ...
    $conn = new mysqli("localhost", "root", "", "demo"); // Change as needed

    // Determine if input is email or phone
    $field = filter_var($identifier, FILTER_VALIDATE_EMAIL) ? 'email' : 'phone';

    // Check if user already exists
    $stmt = $conn->prepare("SELECT id FROM cust WHERE $field = ?");
    ...
    if ($stmt->num_rows > 0) {
      ...
      if ($field === 'email') {
        $stmt = $conn->prepare("INSERT INTO cust (name, email, password) VALUES (?, ?, ?)");
      } else {
        $stmt = $conn->prepare("INSERT INTO cust (name, phone, password) VALUES (?, ?, ?)");
      }
      $stmt->bind_param("sss", $name, $identifier, $hashedPassword);
      if ($stmt->execute()) {
        $success = "Registration successful! <a href='../login/login.php'>Login now</a>";
      } else {
  ...
  <div class="container">
    ...
    <form id="register-form" method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
      <div class="input-wrapper">
        ...
        <input type="text" id="name" name="name" placeholder="Full Name" required value="<?php echo isset($_POST['name']) ? htmlspecialchars($_POST['name']) : ''; ?>">
      </div>
      <div class="input-wrapper" style="margin-top: 1.5rem;">
        ...
        <input type="text" id="identifier" name="identifier" placeholder="E-mail or Phone" required value="<?php echo isset($_POST['identifier']) ? htmlspecialchars($_POST['identifier']) : ''; ?>">
      </div>
      <div class="input-wrapper" style="margin-top: 1.5rem;">
        <!-- Lock SVG icon -->
